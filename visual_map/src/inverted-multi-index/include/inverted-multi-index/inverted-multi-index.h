...
class InvertedMultiIndex {
 public:
  ...
  inline void serialize(
    ...
    for(int i=0; i<frame_indice.size(); i++){
        proto_inverted_multi_index->add_frame_ids(frame_indice[i]);
    }
    for(int i=0; i<track_indice.size(); i++){
        proto_inverted_multi_index->add_track_ids(track_indice[i]);
    }
  }
  ...
  inline void deserialize(
    ...
    for(int i=0; i<proto_inverted_multi_index.frame_ids_size(); i++){
        frame_indice.push_back(proto_inverted_multi_index.frame_ids(i));
    }
    for(int i=0; i<proto_inverted_multi_index.track_ids_size(); i++){
        track_indice.push_back(proto_inverted_multi_index.track_ids(i));
    }
  }
