...
bool exportSessionDataToDatabase(const SegmentedCloud& segmented_cloud, std::string out_addr) {
  return exportSegments(out_addr + kSegmentsFilename, segmented_cloud) &&
      exportFeatures(out_addr + kFeaturesFilename, segmented_cloud);
}

bool importSessionDataFromDatabase(SegmentedCloud* segmented_cloud_ptr, std::string in_addr) {
  return importSegments(in_addr + kSegmentsFilename, segmented_cloud_ptr) &&
      importFeatures(in_addr + kFeaturesFilename, segmented_cloud_ptr);
}
