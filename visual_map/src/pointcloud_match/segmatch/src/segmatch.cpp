...
void SegMatch::processAndSetAsSourceCloud(LocalMapT& local_map) {
...
void SegMatch::setAsTargetCloud() {
  // Overwrite the old target.
  classifier_->setTarget(segmented_target_cloud_);
}
...
PairwiseMatches SegMatch::findMatches() {
    unsigned int track_id=0;
    BENCHMARK_BLOCK("SM.Worker.FindMatches");
    PairwiseMatches candidates;
    if (!segmented_source_clouds_[track_id].empty()) {
        candidates = classifier_->findCandidates(segmented_source_clouds_[track_id], NULL);
    }
  return candidates;
}
...
const PairwiseMatches& SegMatch::recognize(const PairwiseMatches& predicted_matches, Eigen::Matrix4f& g_pose) {
    ...
    // was unsuccessful.
    const std::vector<PairwiseMatches>& candidate_clusters = recognizer->getCandidateClusters();
    ...
    const std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>& candidate_pose = recognizer->getCandidateTransformations();
    g_pose=candidate_pose[0];
    return last_filtered_matches_;
}
...
void SegMatch::get_target_matrix(Eigen::MatrixXf& target_matrix_, std::vector<unsigned int>& indices_list){    
    ...
    if (n_non_empty_views != segmented_target_cloud_.getNumberOfValidSegments()) { 
        LOG(INFO) << "Some segments had empty views";
    }
    target_matrix_.resize(n_non_empty_views, 7);
    unsigned int i = 0u;
    for (std::unordered_map<Id, Segment>::const_iterator it = segmented_target_cloud_.begin(); it != segmented_target_cloud_.end(); ++it) {
        Segment target_segment = it->second;
        if (target_segment.empty()) {
            std::cout<<"get empty setment!!!"<<std::endl;
        }
        indices_list.push_back(it->first);
        target_matrix_.block(i, 0, 1, 7) = target_segment.getLastView().features.rotationInvariantFeaturesOnly().asEigenMatrix().block(0, 0, 1, 7).cast<float>();
        //std::cout<<target_matrix_.row(i)<<std::endl;
        i++;
        
    }
}

void SegMatch::get_match_points( PointICloud& cloud, std::vector<unsigned int>& indices_list){
    size_t cloud_size = 0;
    for (int i=0; i<indices_list.size(); i++){
        Segment segment;
        segmented_target_cloud_.findValidSegmentById(indices_list[i], &segment);
        cloud_size += segment.getLastView().point_cloud_to_publish.size();
    }
    cloud.reserve(cloud_size);
    for (int i=0; i<indices_list.size(); i++){
        Segment segment;
        segmented_target_cloud_.findValidSegmentById(indices_list[i], &segment);
        float segment_color = RVizUtilities::getSegmentColorAsIntensity(indices_list[i]);
        for (const auto& point : segment.getLastView().point_cloud_to_publish) {
