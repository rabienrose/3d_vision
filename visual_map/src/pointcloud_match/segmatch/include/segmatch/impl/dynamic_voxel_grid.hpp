...
std::vector<int> DynamicVoxelGrid<_DVG_TEMPLATE_SPEC_>::insert(const InputCloud& new_cloud, double time) {
...
inline bool DynamicVoxelGrid<_DVG_TEMPLATE_SPEC_>::createVoxel_(
  ...
  if (data.old_voxel != nullptr) {
    ...
    centroid.time=data.old_voxel->centroid->time;
  }
